...
// Turn-by-turn navigation map (MapLibre GL JS + turf in a WebView). Road geometry
// from OSRM; the remaining route stays highlighted while the traveled part is
// trimmed as the courier moves. Exposes setRoute() and updatePosition() to RN.
export function buildNavHtml(theme: Theme): string {
  const c = theme.color;
...
<style>
  html,body,#map{height:100%;margin:0;background:${c.bg};font-family:Inter,system-ui,sans-serif}
  #banner{position:absolute;top:12px;left:12px;right:12px;z-index:5;display:none;align-items:center;gap:12px;
    background:${c.raised};border:1px solid ${c.line2};border-radius:16px;padding:12px 14px;box-shadow:0 8px 30px rgba(0,0,0,.5)}
  #banner .ar{font-size:26px;color:${c.accentT};line-height:1}
  #banner .tx{flex:1;color:${c.ink}}
  #banner .ins{font-size:16px;font-weight:600}
  #banner .dst{font-size:12px;color:${c.ink2};font-family:monospace}
...
<script>
var map=new maplibregl.Map({container:'map',style:{version:8,sources:{c:{type:'raster',tiles:['https://a.basemaps.cartocdn.com/${tiles}/{z}/{x}/{y}.png','https://b.basemaps.cartocdn.com/${tiles}/{z}/{x}/{y}.png'],tileSize:256}},layers:[{id:'c',type:'raster',source:'c'}]},center:[106.84,-6.2],zoom:15,attributionControl:false});
...
map.on('load',function(){
  ...
  map.addLayer({id:'full',type:'line',source:'full',paint:{'line-color':'${c.line2}','line-width':6,'line-opacity':.35}});
  map.addLayer({id:'remain',type:'line',source:'remain',paint:{'line-color':'${c.accentT}','line-width':6,'line-opacity':.95}});
  ready=true;
});
...
function courierEl(){
  ...
  d.innerHTML='<svg width="30" height="30" viewBox="0 0 24 24"><circle cx="12" cy="12" r="11" fill="${c.accent}"/><path d="M12 5 L17 17 L12 14 L7 17 Z" fill="${c.inverse}"/></svg>';
  return d;
}

// Fetch road geometry + step list from OSRM, then draw the route and stop pins.
async function setRoute(fLng,fLat,tLng,tLat,stops){
  ...
  (stops||[]).forEach(function(s){ var e=document.createElement('div'); e.className='pin'; e.style.background=s.color||'${c.danger}';
