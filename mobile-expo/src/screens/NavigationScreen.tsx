...
import { NAV_HTML } from '../lib/navHtml';
...
export default function NavigationScreen({ navigation }: any) {
  ...
  const sendRoute = useCallback(() => {
    ...
    const pins = pending.map((s) => ({ lng: s.order.longitude, lat: s.order.latitude, color: riskColor(s.spoilage_risk) }));
  ...
  }, [current, route, pending]);

  // re-route whenever the active stop changes
  useEffect(() => { if (current && routedFor.current !== current.id) sendRoute(); }, [current?.id, sendRoute]);

  // live GPS → follow + consume route + stream to dashboard
  useEffect(() => {
    if (!perm) return;
    ...
    (async () => {
      ...
      sub = await Location.watchPositionAsync(
        ...
        (p) => {
          ...
          realtime.send({ type: 'gps', lat, lng, heading: hd, speed: (p.coords.speed ?? 0) * 3.6, route_id: route?.id });
          if (routedFor.current !== current?.id) sendRoute(); // first fix → draw route
        },
      );
    })();
    return () => { alive = false; sub?.remove(); };
  }, [perm, route?.id, current?.id, sendRoute]);
        ...
        {current ? (
          <>
            <View style={[s.rowBetween, { marginVertical: 14 }]}>
              ...
              <View style={[s.pill, { backgroundColor: riskColor(current.spoilage_risk) + '28' }]}>
                <View style={[s.dot, { backgroundColor: riskColor(current.spoilage_risk) }]} />
                <Text style={[s.pillText, { color: riskColor(current.spoilage_risk) }]}>{current.spoilage_risk}</Text>
              </View>
            </View>
